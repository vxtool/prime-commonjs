...
Ajax.prototype.beforeRequest = function(options) {
	...
	MAG.mediator.subscribe("ajax-before-" + options.type + "-return", function(obj) {
		me.objAjaxBefore = obj;
	});

	MAG.mediator.publish("ajax-before-" + options.type, options.$form);
};
/**
 * @method data
 */
...
Ajax.prototype.dataSuccess = function(options, data) {
	...
	MAG.mediator.publish("ajax-success-" + options.type, options.$form, data);
...
Ajax.prototype.dataError = function(options, data) {
	MAG.mediator.publish("ajax-error-" + options.type, options.$form, data);
